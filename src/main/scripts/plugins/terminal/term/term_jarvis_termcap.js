// SOURCE FILE: admin.tools/src/main/scripts/terminals/term_jarvis_termcap.js
 ...
 */
term.JarvisTermcap.prototype.output = function(str, opt_vars) {
  let vars;
  if (opt_vars) {
  ...
  return term.JarvisTermcap.replaceVars_(str, vars, this.outputFunctions);
};
 ...
 */
term.JarvisTermcap.prototype.input = function(str, opt_vars) {
  let vars;
  if (opt_vars) {
  ...
  return term.JarvisTermcap.replaceVars_(str, vars, this.inputFunctions);
};

/**
 * The valid variables for term.JarvisTermcap..output()
 */
term.JarvisTermcap.prototype.outputVars = {
  'FG_BOLD': '1',
...
/**
 * The valid functions for term.JarvisTermcap..output()
 */
term.JarvisTermcap.prototype.outputFunctions = {
  'crlf': function(str) {
    return str.replace(/\n/g, '\r\n');
  },
...
/**
 * The valid variables for term.JarvisTermcap..input()
 */
term.JarvisTermcap.prototype.inputVars = {
  'BACKSPACE': '\x7f',
  'DELETE': '\x1b[3~',
  'DOWN': '\x1b[B',
  'END': '\x1b[F',
...
/**
 * The valid functions for term.JarvisTermcap..input()
 */
term.JarvisTermcap.prototype.inputFunctions = {
  'shift': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';2', seq);
  ...
  'meta': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';3', seq);
  ...
  'shift-meta': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';4', seq);
  ...
  'ctrl': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';5', seq);
  ...
  'shift-ctrl': function(ch) {
    if (/\x1b\[/.test(ch))
      return term.JarvisTermcap.modcsi(';6', ch);
  ...
  'ctrl-meta': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';7', seq);
  ...
  'shift-ctrl-meta': function(seq) {
    if (/\x1b\[/.test(seq))
      return term.JarvisTermcap.modcsi(';8', seq);
 ...
 */
term.JarvisTermcap.replaceVars_ = function(str, vars, functions) {
...
term.JarvisTermcap.modcsi = function(mod, seq) {
  if (seq.length === 3) {
...
module.exports = term.JarvisTermcap;
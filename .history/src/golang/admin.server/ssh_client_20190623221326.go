...
func readWriteFromChannel(rc chan *ChanData, nc) {
	for {
		select {
		case cd, ok := <-rc:
			if !ok || cd.Err == io.EOF {
...
func sshInit(ws *Ws) {
	...
	if cmd.Stdin != nil {
		...
		rc := gochan.NewReadonlyChan(stdin, 0, 4096)
		go func() { cmd.Stdin <- rc }()
	}
	...
	if cmd.Stdout != nil {
		...
		wc := gochan.NewWriteonlyChan(stdout, 4096)
		go func() { cmd.Stdout <- wc }()
	}
	...
	if cmd.Stderr != nil {
		...
		wce := gochan.NewWriteonlyChan(stderr, 4096)
